...
func serviceUploadUrl(base *DynamicEndpoint, fileName string) *url.URL {
	return base.Get().ResolveReference(&url.URL{Path: path.Join("upload", fileName)})
}

func serviceDeleteUrl(base *DynamicEndpoint) *url.URL {
	return base.Get().ResolveReference(&url.URL{Path: "delete"})
}
