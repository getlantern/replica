...
func TestDynamicEndpoint(t *testing.T) {
	...
	assert.Equal(t, defaultEndpoint, dynamicEndpoint.Get().String())
	for _, interval := range intervals {
		time.Sleep(defaultSleepDuration + 10*time.Millisecond)
		if interval.shouldFail {
			assert.Equal(t, lastSuccessfulEndpoint, dynamicEndpoint.Get().String())
		} else {
			assert.Equal(t, interval.endpoint, dynamicEndpoint.Get().String())
